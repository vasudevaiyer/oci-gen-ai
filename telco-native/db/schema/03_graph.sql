...
/*
-- Find digital advocates within 2 hops of an urgent signal post author
...
-- Find shortest propagation path between two digital advocates
SELECT *
FROM GRAPH_TABLE ( influencer_network
    MATCH SHORTEST (a IS influencer) -[e IS connects_to]->{1,5} (b IS influencer)
    WHERE a.handle = '@5gmobile_mark'
      AND b.handle = '@navigation_jen'
    COLUMNS (
        a.handle AS source,
        b.handle AS target,
...
-- Service brand propagation: which advocates propagate a service brand through the network
...
FROM GRAPH_TABLE ( influencer_network
    ...
    WHERE b.brand_name = 'SignalBridge Mobile'
    COLUMNS (
        v1.handle, v2.handle,
        e1.relationship_type,
        e2.connection_type,
