/*
 * load_orders.sql
 * 3000 service orders with service line items, varied statuses, and network-signal attribution
 */

SET SERVEROUTPUT ON
PROMPT Loading service orders...
...
BEGIN
    FOR i IN 1..3000 LOOP
        ...
        -- 30% of service orders are associated with a subscriber/network signal
        IF DBMS_RANDOM.VALUE < 0.3 THEN
            SELECT post_id INTO v_social_id
            FROM (
              SELECT post_id
              FROM social_posts
              ORDER BY DBMS_RANDOM.VALUE
        ...
        -- Assign network operations center for non-pending service orders
        ...
        -- Add requested services and supplies
        FOR j IN 1..v_num_items LOOP
            v_qty := FLOOR(DBMS_RANDOM.VALUE(1, 4));

            BEGIN
                SELECT product_id, unit_price INTO v_prod_id, v_price
                FROM (
        ...
        -- Update total service revenue
        UPDATE orders SET order_total = v_total WHERE order_id = v_order_id;

        -- Create field routing/dispatch record for routed or completed requests
        IF v_status IN ('shipped', 'delivered') AND v_center_id IS NOT NULL THEN
            ...
            INSERT INTO shipments (
                ...
                CASE MOD(i, 4)
                    WHEN 0 THEN 'Fiber Crew'
                    WHEN 1 THEN 'RAN Crew'
                    WHEN 2 THEN 'Device Dispatch'
                    ELSE 'Partner Field Crew'
                END,
    ...
    COMMIT;
    DBMS_OUTPUT.PUT_LINE('Service orders loaded: ' || v_count);
END;
/
