...
resource "oci_database_autonomous_database_wallet" "rag_adb" {
  autonomous_database_id = oci_database_autonomous_database.rag_adb.id
  password               = var.adb_admin_password
  generate_type          = "SINGLE"
  base64_encode_content  = true
}

resource "oci_objectstorage_bucket" "adb_wallet" {
  compartment_id = local.resolved_compartment_ocid
  namespace      = data.oci_objectstorage_namespace.tenant.namespace
  name           = "${local.name_prefix}-wallet"
  access_type    = "NoPublicAccess"
  freeform_tags  = merge(local.common_tags, { purpose = "adb-wallet" })
}

resource "oci_objectstorage_object" "adb_wallet" {
  namespace    = data.oci_objectstorage_namespace.tenant.namespace
  bucket       = oci_objectstorage_bucket.adb_wallet.name
  object       = "wallet.zip.b64"
  content      = oci_database_autonomous_database_wallet.rag_adb.content
