...
- Private Object Storage bucket and generated ADB wallet object
...
- Both generated Vault secret names include a Terraform-generated suffix so a fresh
  test stack can be created while a prior stack's secrets are pending deletion.
- If a failed Apply has already persisted a suffix in Terraform state, increment
  `secret_name_nonce` (for example from `1` to `2`) before planning again.
- The ADB wallet is generated by Terraform, stored in a private Object Storage bucket,
  and downloaded by the application VM using its instance principal.
...
- The VM bootstrap creates the application schema with the ADB admin connection, runs the
  application's versioned Node/oracledb migrations, and starts the service.
