...
start_service() {
  ...
  if is_running; then
    printf 'AgentStudio is already running with PID %s\n' "$(<"$PID_FILE")"
    return 0
  fi
  ...
  sleep 1
  if is_running; then
    printf 'AgentStudio started with PID %s\n' "$(<"$PID_FILE")"
    printf 'URL: http://%s:%s\n' "$HOST" "$PORT"
    printf 'Log: %s\n' "$LOG_FILE"
    return 0
  fi

  printf 'AgentStudio failed to start. Check %s\n' "$LOG_FILE" >&2
  return 1
}
...
stop_service() {
  if ! is_running; then
    rm -f "$PID_FILE"
    printf 'AgentStudio is not running\n'
    return 0
  fi
...
status_service() {
  if is_running; then
    printf 'AgentStudio is running with PID %s\n' "$(<"$PID_FILE")"
    printf 'URL: http://%s:%s\n' "$HOST" "$PORT"
    printf 'Log: %s\n' "$LOG_FILE"
    return 0
  fi
